...
var  debug = require('debug')('pangu:top')
    ...
    , config = require('../../../plugin_config/'+sysConfig.province+'/db/conversation/config_Conversation').config
    , chart_list = require('../../../plugin_config/'+sysConfig.province+'/db/conversation/config_Conversation').list
    , transcode_list = require('../../../config/'+sysConfig.province+'/config_coreTranscodeList').coreTranscodeList
    , extend = require('extend');
var auth = require('../../../auth');
var moduleName = require('../../../plugin_config/'+sysConfig.province+'/db/conversation/config_Conversation').moduleName;

module.exports = function (app) {
    app.get('/db/conversation/history.html', auth.requiresLogin, function(req, res) {
        var chartList = req.query.chartList;
        var isWhere = req.query.isWhere||'false';
        var listIndex = chart_list[chartList];
        var listCnt = chart_list[chartList].length;
        res.renderPjax('plugin/db/conversation/history',{chartList:chartList,listIndex:listIndex,listCnt:listCnt,
            coreTranscodeList:transcode_list,isWhere:isWhere,moduleName:moduleName[chartList]});
    });
