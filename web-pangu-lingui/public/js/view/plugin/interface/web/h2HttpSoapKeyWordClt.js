$(function() {
    ...
    function updateInfo() {
        ...
        $("#query_key_processing").show();
        $.ajax({
            type: "get",
            url: "/getH2HttpSoapKeyWordData",
            data: {
                date: $('#value').val(),
                chartList: $('#chartList').val(),
                key: $("#key_word_in").val() || null
            },
            success: function(data){
                if(!data || (data.soapCnt === 0 && data.httpCnt === 0 && data.h2Cnt === 0)){
                    $("#query_key_processing").hide();
    ...
    $('#key_word_in').keydown(function(e){
        if(e.keyCode === 13){
            clearInfo();
            e.preventDefault();
            updateInfo();
    ...
    function setInfo(data, date){
        date = date || $('#value').val();
        $('.query-key-cnt').html('SOAP: '+data.soapCnt+', HTTP: '+data.httpCnt+', H2: '+data.h2Cnt);
        var soap = '';
        var http = '';
        var h2 = '';
        for(var i=0; i< data.soap.length; i++){
            soap += '<tr><td class="text-center">'+ data.soap[i].TRANS_IDO +'</td>';
            soap += '<td class="text-center">'+data.soap[i].SYS_ID +'</td>';
        ...
        $("#query_key_soap > tbody").html(soap);
        for(i=0; i< data.http.length; i++){
            http += '<tr><td class="text-center">'+ data.http[i].TransIDO +'</td>';
            http += '<td class="text-center">'+data.http[i].SYS_ID +'</td>';
            //http += '<td class="text-center">'+data.http[i].SERVICE_NAME +'</td>';
        ...
        $("#query_key_h2 > tbody").html(h2);
        if(data.h2Cnt === 0 ){
            $("#query_key_h2 > tbody").html(noInfo);
        }else{
        ...
        if(data.httpCnt === 0 ){
            $("#query_key_http > tbody").html(noInfo);
        }else{
        ...
        if(data.soapCnt === 0 ){
            $("#query_key_soap > tbody").html(noInfo);
        }else{
